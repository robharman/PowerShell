...
Describe 'Default Tests' {
    it '<Path> Should not violate: <IncludeRule>' -TestCases @(
        Foreach ($ScriptAnalyzerRule in (Get-ScriptAnalyzerRule)) {
            @{
                ...
                Path        =   "$PSScriptRoot\..\Core\Public\Confirm-HomeAdminRights.ps1"
            }
        }
    ) -Skip:($IncludeRule -eq 'PSUseSingularNouns') {
        param(
            $IncludeRule,
            $Path
        )
    ...
    . "$PSScriptRoot\..\Core\Public\Confirm-HomeAdminRights.ps1"

    $GetHelpContent = @{
        HelpInfo                    =   Get-Help -Name Confirm-HomeAdminRights -Full | Select-Object -Property *
    }
    ...
    # Check parameters
    $Params                     =   Get-Help -Name Confirm-HomeAdminRights -Parameter * -ErrorAction SilentlyContinue |
    ...
    $InternalCode = @{
        Code                    =   (Get-Content -Path 'function:/Confirm-HomeAdminRights' -ErrorAction Ignore).Ast
        Params                  =   $Params
    }
...
Describe 'Confirm-HomeAdminRights' {
    BeforeAll {
        . $PSScriptRoot\..\Core\Public\Confirm-HomeAdminRights.ps1
    }

    Context 'Confirm-HomeAdminRights tests' {
        it 'should return a boolean' {
            Confirm-HomeAdminRights | should -BeOfType boolean
        }
    }
}

Describe -tag "RequireAdminOnWindows" "Confirm-HomeAdminRights admin tests"  {
    BeforeAll {
        . $PSScriptRoot\..\Core\Public\Confirm-HomeAdminRights.ps1
    }

    it 'should return true while elevated' {
        Confirm-HomeAdminRights | should -BeTrue
    }
}