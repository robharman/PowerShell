...
Describe 'Default Tests' {
    it '<Path> Should not violate: <IncludeRule>' -TestCases @(
        Foreach ($ScriptAnalyzerRule in (Get-ScriptAnalyzerRule)) {
            @{
                ...
                Path        =   "$PSScriptRoot\..\Core\Public\Confirm-HomeLanConnectivity.ps1"
            }
        }
    ) {
        param(
    ...
    . "$PSScriptRoot\..\Core\Public\Confirm-HomeLanConnectivity.ps1"

    $GetHelpContent = @{
        HelpInfo                    =   Get-Help -Name Confirm-HomeLanConnectivity -Full | Select-Object -Property *
    }
    ...
    # Check parameters
    $Params                     =   Get-Help -Name Confirm-HomeLanConnectivity -Parameter * -ErrorAction SilentlyContinue |
    ...
    $InternalCode = @{
        Code                    =   (Get-Content -Path 'function:/Confirm-HomeLanConnectivity' -ErrorAction Ignore).Ast
        Params                  =   $Params
    }
...
describe 'Confirm-HomeLanConnectivity tests' {
    BeforeAll {
        . $PSScriptRoot\..\Core\Public\Confirm-HomeLanConnectivity.ps1
    }

    it 'should return a boolean' {
        Confirm-HomeLanConnectivity | should -BeOfType boolean
    }

    it 'should return false when testing an random host' {
        Confirm-HomeLanConnectivity -Target (-join ((97..122) | Get-Random -Count 16 | ForEach-Object {[char]$_})) | should -Be $False
    }

    it 'should return false when testing an random domain' {
        Confirm-HomeLanConnectivity -Domain ((-join ((97..122) | Get-Random -Count 16 | ForEach-Object {[char]$_})) + ".com") | should -Be $False
    }

    it 'should return true when testing a valid domain' {
        mock Test-Connection {return $True}
        Confirm-HomeLanConnectivity | should -be $true
    }

    it 'should return false when testing an invalid domain' {
        mock Test-Connection {return $false}
        Confirm-HomeLanConnectivity | should -be false
    }
