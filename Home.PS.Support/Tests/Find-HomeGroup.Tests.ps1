...
Describe 'Default Tests' {
    it '<Path> Should not violate: <IncludeRule>' -TestCases @(
        Foreach ($ScriptAnalyzerRule in (Get-ScriptAnalyzerRule)) {
            @{
                ...
                Path        =   "$PSScriptRoot\..\Support\Public\Find-HomeGroup.ps1"
            }
        }
    ) {
        param(
    ...
    . "$PSScriptRoot\..\Support\Public\Find-HomeGroup.ps1"

    $GetHelpContent = @{
        HelpInfo                    =   Get-Help -Name Find-HomeGroup -Full | Select-Object -Property *
    }
    ...
    # Check parameters
    $Params                     =   Get-Help -Name Find-HomeGroup -Parameter * -ErrorAction SilentlyContinue |
    ...
    $InternalCode = @{
        Code                    =   (Get-Content -Path 'function:/Find-HomeGroup' -ErrorAction Ignore).Ast
        Params                  =   $Params
    }
