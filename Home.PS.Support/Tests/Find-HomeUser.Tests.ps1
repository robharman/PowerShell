...
Describe 'Default Tests' {
    it '<Path> Should not violate: <IncludeRule>' -TestCases @(
        Foreach ($ScriptAnalyzerRule in (Get-ScriptAnalyzerRule)) {
            @{
                ...
                Path        =   "$PSScriptRoot\..\Support\Public\Find-HomeUser.ps1"
            }
        }
    ) {
        param(
    ...
    . "$PSScriptRoot\..\Support\Public\Find-HomeUser.ps1"

    $GetHelpContent = @{
        HelpInfo                    =   Get-Help -Name Find-HomeUser -Full | Select-Object -Property *
    }
    ...
    # Check parameters
    $Params                     =   Get-Help -Name Find-HomeUser -Parameter * -ErrorAction SilentlyContinue |
    ...
    $InternalCode = @{
        Code                    =   (Get-Content -Path 'function:/Find-HomeUser' -ErrorAction Ignore).Ast
        Params                  =   $Params
    }
