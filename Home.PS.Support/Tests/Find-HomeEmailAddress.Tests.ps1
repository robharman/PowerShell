...
Describe 'Default Tests' {
    it '<Path> Should not violate: <IncludeRule>' -TestCases @(
        Foreach ($ScriptAnalyzerRule in (Get-ScriptAnalyzerRule)) {
            @{
                ...
                Path        =   "$PSScriptRoot\..\Support\Public\Find-HomeEmailAddress.ps1"
            }
        }
    ) {
        param(
    ...
    . "$PSScriptRoot\..\Support\Public\Find-HomeEmailAddress.ps1"

    $GetHelpContent = @{
        HelpInfo                    =   Get-Help -Name Find-HomeEmailAddress -Full | Select-Object -Property *
    }
    ...
    # Check parameters
    $Params                     =   Get-Help -Name Find-HomeEmailAddress -Parameter * -ErrorAction SilentlyContinue |
    ...
    $InternalCode = @{
        Code                    =   (Get-Content -Path 'function:/Find-HomeEmailAddress' -ErrorAction Ignore).Ast
        Params                  =   $Params
    }
...
describe 'Find-HomeEmailAddress tests' {
    context "Mock tests" {
        beforeeach {
        ...
        it 'should return the AD Object' -TestCases @(
            @{
                SearchParam =   'null@nullmail.com'
                MockObject  =   @{
                    DistinguishedName   =   'ou=ou,dc=base,dc=tld'
                    Name                =   'User Name'
                    SAMAccountName      =   'UName'
                    ProxyAddresses      = @('user@email.com', 'uname@email.com')
                }
            }
        ) {
            param (
            ...
            mock -CommandName Get-ADObject -MockWith { return $MockObject }

            Find-HomeEmailAddress $SearchParam | Should -Be $MockObject

            Assert-MockCalled -CommandName Get-ADObject 1
        }
    }
}

describe 'Find-HomeEmailAddress Failure tests' {
    BeforeAll {
        function Get-ADObject {}
    ...
    it 'should throw when you give the invalid email <SearchParam>' -TestCases @(
        @{ SearchParam = 'null'},
        @{ SearchParam = 'null@throw'},
        @{ SearchParam = 'null@'}
    ){
        param (
            $SearchParam
        ...
        { Find-HomeEmailAddress $SearchParam } | Should -Throw
    }

    it 'should throw when there are no objects found' {
        { Find-HomeEmailAddress 'null@null.null' } | Should -Throw 'No users found'
