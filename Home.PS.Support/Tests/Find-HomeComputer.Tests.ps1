...
Describe 'Default Tests' {
    it '<Path> Should not violate: <IncludeRule>' -TestCases @(
        Foreach ($ScriptAnalyzerRule in (Get-ScriptAnalyzerRule)) {
            @{
                ...
                Path            =   "$PSScriptRoot\..\Support\Public\Find-HomeComputer.ps1"
            }
        }
    ) {
        param(
    ...
    . "$PSScriptRoot\..\Support\Public\Find-HomeComputer.ps1"

    $GetHelpContent = @{
        HelpInfo                =   Get-Help -Name Find-HomeComputer -Full | Select-Object -Property *
    }
    ...
    # Check parameters
    $Params                     =   Get-Help -Name Find-HomeComputer -Parameter * -ErrorAction SilentlyContinue |
    ...
    $InternalCode = @{
        Code                    =   (Get-Content -Path 'function:/Find-HomeComputer' -ErrorAction Ignore).Ast
        Params                  =   $Params
    }
