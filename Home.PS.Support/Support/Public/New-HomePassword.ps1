Function New-HomePassword {
    <#
    .SYNOPSIS
        Creates a human usable pseudo-random password based on Diceware.
    .DESCRIPTION
        Creates a human usable pseudo-random password based on Diceware with some extra symbols thrown in, and maybe
    ...
    .EXAMPLE
        New-HomePassword -Simple

        Returns a simple password like: MINES%01919
    .EXAMPLE
        New-HomePassword -DictionaryDirectory "\\fileserver\passworddictionaries\" -NumberOfDice 5 -NumberOfWords 5
    ...
    if ($Simple) {
        ...
        if ($Password -cmatch "^[^A-Z]*$"){
            ...
            $Password = New-HomePassword -NumberOfDice $NumberOfDice -NumberOfWords $NumberOfWords -DictionaryDirectory $DictionaryDirectory -SuppressWarning:$SuppressWarning
        }
